...
do
  ...
  if [[ $RUNNING -gt 0 ]]; then
    ...
    #zpool status $pool | grep "to go"
    continue
  fi

  #ERRORS=$(zpool status $pool | grep "scan:" | grep -v "in progress" | grep -v " 0 errors" | wc -l)
  ERRORS=$(zpool status $pool | grep "scan:" | grep -v " 0 errors" | wc -l)
  if [[ $ERRORS -gt 0 ]]; then
